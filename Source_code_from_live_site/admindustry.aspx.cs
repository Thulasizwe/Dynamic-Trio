...
public partial class Default2 : System.Web.UI.Page
{
   // private DAL_ DAL = DAL_.Instance;
    ...
    {
        DataTable sp_LoadIndustry = new DataTable();
        using (SqlConnection sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["ManDash"].ConnectionString))
        {
            using (SqlCommand cmd = new SqlCommand("sp_LoadIndustry", sqlcon))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    da.Fill(sp_LoadIndustry);
                }
            }
        }
        ASPxGridView1.cxGridSetup(sp_LoadIndustry);
